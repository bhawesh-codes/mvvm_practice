...
class HomeViewModel extends ChangeNotifier {
  ...
  Future<void> fetchBooks() async {
    ...
    if (!hasLoadedSlugs) {
      favoriteSlugs = await favRepo.getSlugs(); // ✅ repo handles prefs
      hasLoadedSlugs = true;
    }
  ...
  Future<void> toggleFavorite(Result book) async {
    await favRepo.toggle(book); // ✅ repo handles logic
  ...
  bool isFavorite(Result book) {
    return favRepo.isFavorite(book, favoriteSlugs); // ✅ repo handles check
  }

  Future<void> refreshFavoriteSlugs() async {
    favoriteSlugs = await favRepo.getSlugs();
    notifyListeners();
  }
